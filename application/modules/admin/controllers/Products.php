...
class Products extends CI_Controller {
    ...
    {
        $params['title'] = 'Generate Kupon Belanja';

        $this->load->view('header', $params);
        $this->load->view('products/generate-coupons', $params);
        $this->load->view('footer');
    }
    ...
    {
        /*
            collect data from input data to variable
        */
        $urutanHariBesar = $this->input->post('hariBesar');
        $urutanAkhirBulan = $this->input->post('akhirBulan');
        $urutanMingguan = $this->input->post('mingguan');
        ...
        */
        $arraySchedulle = [
            [
                "type" => "hari_besar",
                "value" => $urutanHariBesar,
            ],
            [
                "type" => "akhir_bulan",
                "value" => $urutanAkhirBulan,
            ],
            [
                "type" => "mingguan",
                "value" => $urutanMingguan,
            ],
        ];

        /*
            order array asc by value schedulle
        */
        $keys = array_column($arraySchedulle, 'value');
        array_multisort($keys, SORT_ASC, $arraySchedulle);

        foreach ($arraySchedulle as $key => $data) {
            if($data["type"] == "mingguan"){
                $this->generateKuponMingguan();
            }

            if($data["type"] == "akhir_bulan"){
                $this->generateKuponAkhirBulan();
            }

            if($data["type"] == "hari_besar"){
                $this->generateKuponHariBesar();
            }
        }

        redirect('admin/products/coupons');
    }

    function generateKuponMingguan()
    {
        /*
            get first day of the week
        */
        $firstDay = date('Y-m-d', strtotime("this week"));
        $codeDay = date('YmdHis', strtotime("this week"));
        $end = date('Y-m-d', strtotime($firstDay. ' + 4 days'));
    ...
    function generateKuponAkhirBulan()
    {
        $date = new DateTime('now');
        $date->modify('last day of this month');
        $firstDay = $date->format('Y-m-d');
        $codeFirst = $date->format('YmdHis');
        $end = date('Y-m-d', strtotime($firstDay. ' + 7 days'));
    ...
    function generateKuponHariBesar()
    {
        $curlSession = curl_init();
        curl_setopt($curlSession, CURLOPT_URL, 'https://api-harilibur.vercel.app/api?month=2&year=2023');
        ...
        foreach ($jsonData as $key => $value) {
            $holiday_date = substr($value->holiday_date, 0, -1);
            $holiday_name = substr($value->holiday_name, 0, -1);
            $is_national_holiday = substr($value->is_national_holiday, 0, -1);

            $name = "Kupon ".$holiday_name;
            $firstDay = $holiday_date;
            $code = "HB-".date('YmdHis', strtotime($firstDay));
            $end = date('Y-m-d', strtotime($firstDay. ' + 7 days'));
